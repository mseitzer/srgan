#!/usr/bin/env python
"""Download and setup the Set14 dataset

We are using the Set14 compilation provided by Huang, Singh & Ahuja:
Single Image Super-Resolution From Transformed Self-Exemplars (2015)
(https://github.com/jbhuang0604/SelfExSR)
The original Set14 images come from Zeyde, Elad & Protter: On Single Image
Scale-Up using Sparse-Representations (2010).
"""
import argparse
import os
import shutil
...
DATASET_URL = ('https://uofi.box.com/shared/static/'
               'igsnfieh4lz68l926l8xbklwsnnk8we9.zip')
FILENAME = 'Set14_SR.zip'

parser = argparse.ArgumentParser(description='Generate Set14 dataset')
parser.add_argument('-f', '--force', action='store_true',
                    help='Remove existing dataset directory')
parser.add_argument('--dest-path', default='../../resources/data/Set14',
                    help='Path to output folder')
parser.add_argument('--from-zip', default=None,
                    help='Zipfile of the dataset to extract from')


def download_and_extract_set14(dest_path, from_zip_file=None):
  ...
  with ZipFile(zip_file) as myzip:
   for img in (img for img in myzip.infolist()
     ...
     img.filename = os.path.relpath(img.filename, 'Set14')
...
def main(argv):
  ...
  if not os.path.isdir(args.dest_path):
    download_and_extract_set14(args.dest_path, args.from_zip)
